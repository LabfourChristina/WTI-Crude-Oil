...
(function () {
  ...
  function keepView() {
    ...
    if (followRight) leftIdx = total - vc;
    leftIdx = Math.max(0, Math.min(leftIdx, Math.max(0, total - vc)));
  }
  ...
  function idxOfX(x) {
    ...
    return Math.max(0, Math.min(dayIdx, i));
  }
  ...
  function onWheel(e) {
    ...
    const anchorIdx = idxOfX(mx);          // 缩放前鼠标下蜡烛
    spacing = Math.max(3, Math.min(26, spacing * factor));
    const total = dayIdx + 1;
    const vc = visibleCount();             // 缩放后可见数（新 spacing）
    const relNew = (mx - PAD.left) / spacing; // 缩放后鼠标相对偏移
    followRight = false;
    leftIdx = anchorIdx - Math.floor(relNew);
    leftIdx = Math.max(0, Math.min(leftIdx, Math.max(0, total - vc)));
    if (leftIdx >= total - vc) followRight = true;
    draw();
  }
  function onDown(e) {
    dragging = true;
  ...
  function onMove(e) {
    ...
    if (dragging) {
      ...
      const vc = visibleCount();
      const total = dayIdx + 1;
      leftIdx -= Math.round(dx / spacing);
      leftIdx = Math.max(0, Math.min(leftIdx, Math.max(0, total - vc)));
      followRight = leftIdx >= total - vc;
    } else {
  ...
  function onUp() { dragging = false; draw(); }
  ...
  const Chart = {
    ...
    render(i) {
      if (typeof i === "number") {
        if (i !== dayIdx && i > dayIdx) followRight = true; // 新日自动跟随
        dayIdx = i;
      }
      resize();
      draw();
