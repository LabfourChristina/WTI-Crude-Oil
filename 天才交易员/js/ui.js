...
(function () {
  ...
  const el = {
    ...
    net: $("netWorth"),
    newsDot: $("newsDot"),
    roof: document.querySelector(".go-roof"),
    // 持仓
  ...
  function renderAll() {
    ...
    el.days.textContent = G.daysLeft();
  ...
  function showGameOver(reason) {
    if (reason === "round") {
      // 满 365 天正常收官：最终资产 = 净值 + 余额 - 应还贷款
      const left = G.cash() + G.netWorth() - G.owed();
      const fmt = left.toLocaleString("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      $("goTitle").textContent = "交易结束，恭喜你活到了最后！";
      $("goText").innerHTML =
        (left <= G.START_CASH + 0.005
          ? "你最后剩下 " + fmt + " 元，你保住了部分资金，已经比大多数人强了！"
          : "你最后剩下 " + fmt + " 元，你居然还赚到了钱，真是人才！");
      if (el.roof) el.roof.textContent = "🏆";
    } else {
  ...
  bind();
  Chart.init({
    open: $("lblOpen"), high: $("lblHigh"), low: $("lblLow"), close: $("lblClose"),
    pct: $("lblPct"), date: $("lblDate")
  });
  renderAll();
})();
